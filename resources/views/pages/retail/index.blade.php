...
@section('content')
                        ...
                        <div class="pos-product-grid">
                            @foreach ($products as $product)
                                <article class="product-card" data-product-id="{{ $product['id'] }}" onclick="stackProduct(event, this)">
                                    ...
                                    <div class="sold-row">Sold: <span>{{ $product['sales'] }}</span></div>
    ...
    <script>
        ...
        function updatePrice(productId, priceValue) {
            const amount = parseFloat(priceValue || '0');
            const node = document.getElementById(`price_${productId}`);
            if (node) {
